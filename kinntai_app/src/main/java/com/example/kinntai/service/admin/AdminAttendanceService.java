...
public class AdminAttendanceService {
    ...
    public AttendanceStatsResponse getTodayAttendanceStats() {
        ...
        List<User> filteredUsers = allUsers.stream()
                                  .filter(user -> !user.getRole().name().equals("ADMIN")) // ★修正: enumのname()メソッドで比較
    ...
    public List<TodayAttendanceResponse> getTodayAttendanceList() {
        ...
        for (User user : filteredUsers) { // フィルターされたユーザーリストを使用
            ...
            if (attendance != null) {
                ...
                response.setTodayClockIn(attendance.getClockIn() != null ? 
                    attendance.getClockIn().toLocalTime().toString() : null);
                response.setTodayClockOut(attendance.getClockOut() != null ? 
                    attendance.getClockOut().toLocalTime().toString() : null);
                ...
                if (attendance.getClockIn() != null && attendance.getClockOut() != null) {
                    if (attendance.getOvertimeMinutes() != null && attendance.getOvertimeMinutes() > 0) {
                        ...
                        response.setOvertimeType(response.getOvertimeType() != null ? response.getOvertimeType() : "late");
                    } else {
                    ...
                    response.setStatus("present"); 
                    response.setOvertimeHours(attendance.getOvertimeMinutes() != null ? attendance.getOvertimeMinutes() / 60.0 : 0.0);
                    response.setOvertimeType(null);
                } else {
    ...
    public List<TodayAttendanceResponse> getAttendanceByPeriod(LocalDate startDate, LocalDate endDate, Long userId) {
        ...
        if (userId != null) {
            ...
            attendances = attendanceRepository.findByUser_IdAndDateBetweenOrderByDateAsc(userId, startDate, endDate).stream()
     ...
     */
    public List<TodayAttendanceResponse> getAttendanceByPeriodAndUser(LocalDate startDate, LocalDate endDate, Long userId) {
        List<Attendance> attendances = attendanceRepository.findByUser_IdAndDateBetweenOrderByDateAsc(userId, startDate, endDate).stream()
                                            .filter(attendance -> !attendance.getUser().getRole().name().equals("ADMIN")) // ★修正: enumのname()メソッドで比較
                                            .collect(Collectors.toList());
        attendances.forEach(this::recalculateWorkingHours); 
        return convertToTodayAttendanceResponse(attendances);
    ...
    public List<TodayAttendanceResponse> getUnclockedOutUsers(LocalDate date) {
        List<Attendance> unclockedAttendances = attendanceRepository.findByDateAndClockInNotNullAndClockOutIsNull(date).stream()
                                                    .filter(attendance -> !attendance.getUser().getRole().name().equals("ADMIN")) // ★修正: enumのname()メソッドで比較
                                                    .collect(Collectors.toList());
        ...
        if (attendance.getClockIn() != null) {
            ...
            if (user.getLocation() != null) {
                ...
                if (regularEndTime.isAfter(regularStartTime)) {
                    long regularTotalDuration = java.time.Duration.between(regularStartTime, regularEndTime).toMinutes();
